...
class _CycleScreenState extends State<CycleScreen> {
  ...
  Widget _buildCalendarView(Map<String, dynamic> data) {
    ...
    final DateTime? lastPeriodStart = (data['lastPeriodStart'] as Timestamp?)
        ?.toDate();
    final DateTime? fertilityWindowStart =
        (data['fertilityWindowStart'] as Timestamp?)?.toDate();
    final DateTime? fertilityWindowEnd =
        (data['fertilityWindowEnd'] as Timestamp?)?.toDate();
      ...
      child: Column(
        ...
        children: [
          ...
          _buildInfoTile(
            ...
            title: 'Τρέχων Κύκλος',
            subtitle:
                'Κύκλος $cycleLength ημερών • Περίοδος $periodDuration ημερών',
          ),
          if (lastPeriodStart != null)
            _buildInfoTile(
              icon: Icons.event,
              title: 'Τελευταία Περίοδος',
              subtitle:
                  '${lastPeriodStart.day}/${lastPeriodStart.month}/${lastPeriodStart.year}',
            ),
          _buildInfoTile(
            icon: Icons.calendar_today,
            title: 'Πρόβλεψη Επόμενης Περιόδου',
            subtitle:
          ...
          if (fertilityWindowStart != null && fertilityWindowEnd != null)
            _buildInfoTile(
              icon: Icons.spa,
              title: 'Παράθυρο Γονιμότητας',
              subtitle:
                  '${fertilityWindowStart.day}/${fertilityWindowStart.month}/${fertilityWindowStart.year} - ${fertilityWindowEnd.day}/${fertilityWindowEnd.month}/${fertilityWindowEnd.year}',
            ),
          ...
          SizedBox(
            ...
            child: ElevatedButton.icon(
              ...
              onPressed: _isFutureDate(_selectedCalendarDate)
                  ? null
                  : () => _openDailyLogScreen(data),
              icon: const Icon(Icons.add, color: Colors.white),
              label: const Text(
                'ΚΑΤΑΓΡΑΦΗ ΝΕΩΝ ΔΕΔΟΜΕΝΩΝ',
                style: TextStyle(color: Colors.white),
              ),
            ),
          ),
  ...
  Widget _buildCalendarDayCell({
    ...
    final isActualPeriodDay = _isInDateRange(
      date,
      lastPeriodStart,
      periodDuration,
    );
    final isPredictedPeriodDay = _isInDateRange(
      date,
      nextPeriod,
      periodDuration,
    );
    ...
    bool isPastPeriodDay = pastPeriods.any(
      (p) =>
          p['start'] != null &&
          p['duration'] != null &&
          _isInDateRange(
    ...
    final isFertileDay =
        ...
        !date.isBefore(_dateOnly(fertilityWindowStart)) &&
        !date.isAfter(_dateOnly(fertilityWindowEnd));
    final isOvulationDay = _isSameDay(
      date,
    ...
    if (lastPeriodStart != null && cycleLength > 0) {
      final difference = _dateOnly(
        date,
      ).difference(_dateOnly(lastPeriodStart)).inDays;
      ...
      onTap: () {
        ...
        if (_isFutureDate(date)) {
          ScaffoldMessenger.of(context).showSnackBar(
            const SnackBar(
              content: Text(
                'Δεν μπορείτε να κάνετε καταγραφή για μελλοντική ημερομηνία.',
              ),
              backgroundColor: Colors.orange,
            ),
          );
        } else {
          _openDailyLogScreen(profileData);
        }
      },
      child: Container(
        ...
        decoration: BoxDecoration(
          ...
          border: cellBorder ?? Border.all(color: Colors.grey.shade100),
            ...
            if (cycleDay != null)
              Align(
                ...
                child: Text(
                  '$cycleDay',
                  style: TextStyle(
                    color: isSelected || isActualPeriodDay || isPastPeriodDay
                        ? Colors.white70
  ...
  bool _isFutureDate(DateTime date) =>
      _dateOnly(date).isAfter(_dateOnly(DateTime.now()));
