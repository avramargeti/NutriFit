...
class _DailyLogScreenState extends State<DailyLogScreen> {
  ...
  final List<String> _flowIntensityOptions = [
    'Ελαφριά',
    'Κανονική',
    'Βαριά',
    'Καμία ροή',
  ...
  final List<String> _cycleLogSymptoms = [
    'Κράμπες',
    'Πόνος στη μέση',
    'Πονοκέφαλος',
    'Ατονία',
  ...
  void initState() {
    ...
    _checkAndPrepopulateData();
  }

  Future<void> _checkAndPrepopulateData() async {
    try {
      final doc = await _cycleService.getEntryForDate(selectedDate);
      if (doc != null && mounted) {
        final data = doc.data() as Map<String, dynamic>;
        setState(() {
          existingDocId = doc.id;
          selectedFlow = data['flowIntensity'] ?? 'Ελαφριά';
          selectedMood = data['mood'] ?? 'Ουδέτερη';
          selectedLogSymptoms.addAll(List<String>.from(data['symptoms'] ?? []));
          isPeriodStart = data['cycleEvent'] == 'periodStart';
  ...
  Future<void> _deleteEntry() async {
    ...
    bool? confirmDelete = await showDialog<bool>(
      ...
      builder: (context) => AlertDialog(
        title: const Text('Διαγραφή Δεδομένων;'),
        content: const Text(
          'Είστε σίγουρες ότι θέλετε να διαγράψετε την καταγραφή αυτής της ημέρας;',
        ),
        actions: [
          TextButton(
    ...
    try {
      await _cycleService.deleteCycleEntry(existingDocId!);
      messenger.showSnackBar(
        const SnackBar(
          content: Text('Η καταγραφή διαγράφηκε επιτυχώς!'),
  ...
  Future<void> _saveEntry() async {
    ...
    try {
      ...
      DateTime? lastPeriodStart = _parseDate(
        widget.profileData['lastPeriodStart'],
      );
      DateTime? nextPeriodPredicted = _parseDate(
        widget.profileData['nextPeriodPredicted'],
      );

      if (nextPeriodPredicted == null && lastPeriodStart != null) {
        nextPeriodPredicted = lastPeriodStart.add(
          Duration(days: currentCycleLength),
        );
      }
        ...
        if (deviation >= 4) {
          bool? isConfirmed = await showDialog<bool>(
            ...
            builder: (context) => AlertDialog(
              ...
              content: Text(
                ...
                'Πρόκειται για κανονική έναρξη νέου κύκλου ή για μεμονωμένο συμβάν;',
              ),
              actions: [
                TextButton(
                  onPressed: () => Navigator.of(context).pop(false),
          ...
          if (isConfirmed) {
            ...
            if (actualDaysPassed > 0) {
              finalCycleLength = ((currentCycleLength + actualDaysPassed) / 2)
                  .round();
            }
          } else {
            isPeriodStart = false;
      ...
      messenger.showSnackBar(
        const SnackBar(
          content: Text('Η καταγραφή ενημερώθηκε επιτυχώς!'),
          backgroundColor: Colors.green,
        ),
      );
    } catch (e) {
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text('Προέκυψε σφάλμα: $e'),
  ...
  Widget build(BuildContext context) {
    return AlertDialog(
      title: Text(
        existingDocId != null ? 'Τροποποίηση Καταγραφής' : 'Καταγραφή Κύκλου',
      ),
      content: _isLocalLoading
          ? const SizedBox(
              height: 100,
                  ...
                  children: [
                    ...
                    Container(
                      ...
                      child: SwitchListTile(
                        ...
                        subtitle: const Text(
                          'Το τέλος υπολογίζεται αυτόματα.',
                        ...
                        onChanged: (val) {
                          setState(() {
                            isPeriodStart = val;
                            if (val && selectedFlow == 'Καμία ροή') {
                              selectedFlow = 'Κανονική';
                            }
                          });
                        },
                      ),
                    ...
                    DropdownButtonFormField<String>(
                      ...
                      decoration: const InputDecoration(
                        labelText: 'Ένταση Ροής',
                      ),
                      items: _flowIntensityOptions
                          .map(
                    ...
                    Wrap(
                      ...
                      children: _cycleLogSymptoms.map((symptom) {
                        final isSelected = selectedLogSymptoms.contains(
                          symptom,
                        );
                        return FilterChip(
                          label: Text(symptom),
                          ...
                          onSelected: (selected) => setState(
                            () => selected
                                ? selectedLogSymptoms.add(symptom)
                                : selectedLogSymptoms.remove(symptom),
                          ),
                        );
                      }).toList(),
                    ),
      ...
      actions: [
        if (existingDocId != null && !_isLocalLoading)
          TextButton(
        ...
        ElevatedButton(
          style: ElevatedButton.styleFrom(backgroundColor: sageGreen),
          onPressed: _isLocalLoading ? null : _saveEntry,
          child: Text(
            existingDocId != null ? 'ΕΝΗΜΕΡΩΣΗ' : 'ΑΠΟΘΗΚΕΥΣΗ',
