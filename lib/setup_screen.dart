...
class _SetupScreenState extends State<SetupScreen> {
  ...
  Future<void> _submitForm() async {
    ...
    if (!_formKey.currentState!.validate() || _selectedLastPeriodDate == null || _cycleLengthController.text.isEmpty || _periodDurationController.text.isEmpty) {
  ...
  Future<void> _saveDefaultCycleSettings() async {
    ...
    try {
      ...
      if (mounted) ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Αποθηκεύτηκαν προεπιλεγμένες τιμές.'), backgroundColor: Colors.green));
    } finally {
      if (mounted) setState(() => _isLoading = false);
    }
  }
        ...
        child: Column(
          ...
          children: [
            ...
            ListTile(
              ...
              subtitle: Text(_selectedLastPeriodDate == null ? 'Επιλέξτε ημερομηνία' : '${_selectedLastPeriodDate!.day}/${_selectedLastPeriodDate!.month}/${_selectedLastPeriodDate!.year}'),
            ...
            const Divider(),
            TextFormField(controller: _cycleLengthController, enabled: !_usesUnknownCycleDetails, keyboardType: TextInputType.number, decoration: const InputDecoration(labelText: 'Μέση Διάρκεια Κύκλου (π.χ. 28 μέρες)')),
            const SizedBox(height: 15),
            TextFormField(controller: _periodDurationController, enabled: !_usesUnknownCycleDetails, keyboardType: TextInputType.number, decoration: const InputDecoration(labelText: 'Μέση Διάρκεια Περιόδου (π.χ. 5 μέρες)')),
            ...
            Wrap(
              ...
              children: _commonSymptoms.map((symptom) {
                ...
                return FilterChip(
                  ...
                  onSelected: (selected) => setState(() => selected ? _selectedSymptoms.add(symptom) : _selectedSymptoms.remove(symptom)),
                );
              }).toList(),
            ),
